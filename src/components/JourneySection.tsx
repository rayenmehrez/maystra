        ...
        <div className="relative max-w-4xl mx-auto">
          ...
          <div className="absolute top-0 bottom-0 right-1/2 translate-x-1/2 w-[3px] bg-border rounded-full hidden md:block">
            <motion.div
              ...
              style={{ background: "linear-gradient(to bottom, hsl(263 70% 58%), hsl(270 60% 75%), hsl(263 70% 58%))" }}
            />
          </div>
          ...
          <div className="flex flex-col gap-16 md:gap-20">
            {months.map((month, i) => (
              <RoadmapNode key={month.num} month={month} index={i} isInView={isInView} />
            ))}
          </div>
        </div>
      </div>
...
const RoadmapNode = ({ month, index, isInView }: NodeProps) => {
  ...
  return (
    <>
      ...
      <div className="hidden md:grid grid-cols-[1fr_56px_1fr] items-center gap-0">
        {/* Right card slot */}
        <motion.div
          initial={{ opacity: 0, x: isRight ? 60 : 0 }}
        ...
        <div className="flex justify-center">
          <motion.div
            ...
            transition={{ duration: 0.5, delay: 0.3 + index * 0.3, type: "spring", stiffness: 200 }}
      ...
      >
        <motion.div
          ...
          transition={{ duration: 0.4, delay: 0.3 + index * 0.25, type: "spring" }}
...
const CardContent = ({ month, Icon }: { month: (typeof months)[number]; Icon: React.ElementType }) => (
  <motion.div
    whileHover={{ y: -4, boxShadow: "0 12px 36px hsl(263 70% 58% / 0.15)" }}
