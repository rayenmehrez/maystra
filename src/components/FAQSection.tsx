      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {faqs.map((faq, i) => (
                <AccordionItem
                  key={i}
                  value={`faq-${i}`}
                  className="bg-card rounded-xl border border-border shadow-purple px-6 data-[state=open]:shadow-purple-lg transition-shadow"
                >
        ...
        <AnimatedSection delay={0.4}>
          <div className="text-center mt-12">
            <p className="text-muted-foreground text-sm mb-4">لديك سؤال آخر؟</p>
            <a
              href="https://abeeralmatooq.com/%d9%85%d9%86%d9%87%d8%ac-%d8%a7%d9%84%d9%85%d8%a7%d9%8a%d8%b3%d8%aa%d8%b1%d8%a7/"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block bg-primary text-primary-foreground font-bold text-base px-8 py-3.5 rounded-full hover:scale-105 transition-transform duration-300 shadow-purple"
            >
              تواصلي معنا
            </a>
          </div>
